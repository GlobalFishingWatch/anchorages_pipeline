...
for msg in examples_msgs:
    msg["timestamp"] = (
        datetime.datetime.strptime(msg["timestamp"], "%Y-%m-%d %H:%M:%S.%f UTC")
...
example_records = [
    VesselRecord.tagged_from_msg(x)[1] for x in examples_msgs if is_location_message(x)
]
...
def test_date_parsing_records():
    msgs = [
        {"ident": 123, "timestamp": "2021-04-26 06:00:12.0000 UTC"},
        {"ident": 123, "timestamp": "2021-05-04 12:20:42.798437 UTC"},
    ]
    for msg in msgs:
        msg["timestamp"] = (
            datetime.datetime.strptime(msg["timestamp"], "%Y-%m-%d %H:%M:%S.%f UTC")
    ...
    assert [InvalidRecord.from_msg(x) for x in msgs] == [
        InvalidRecord(
            identifier=123, timestamp=datetime.datetime(2021, 4, 26, 6, 0, 12, tzinfo=pytz.UTC)
        ),
        InvalidRecord(
            identifier=123,
            timestamp=datetime.datetime(2021, 5, 4, 12, 20, 42, 798437, tzinfo=pytz.UTC),
        ),
    ]
...
def test_has_valid_location():
    assert [has_valid_location(x) for x in examples_msgs if is_location_message(x)] == [
        True,
        True,
        False,
    ]
