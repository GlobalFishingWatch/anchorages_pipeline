...
def create_voyage(visits):
    ...
    def has_duration(v1, v2):
        return v1 is None or v1["end_timestamp"] < v2["start_timestamp"]

    for visit in visits:
        for c in [2, 3, 4]:
            if (visit and visit["confidence"] >= c and has_duration(previous_visit[c], visit)) or (
                not visit and previous_visit[c]
            ):
...
def build_voyage(trip_confidence, start_visit, end_visit):
    ...
    voyage = {
        ...
        "trip_id": f"{voyage_ssvid}-{voyage_vessel_id}"
        + (f"-{CreateVoyages.trip_id_hex(voyage_trip_start)}" if start_visit else ""),
    }
    return voyage
...
class CreateVoyages(beam.PTransform):
    ...
    def trip_id_hex(d1: dt.datetime) -> str:
        ...
        duration = (
            lambda d1: (
                (d := d1 - CreateVoyages.DURATION_START).days * CreateVoyages.DAYS_SECONDS
                + d.seconds
            )
            * 1000
            + d.microseconds // 1000
    ...
    def expand(self, pcoll):
        return pcoll | self.create_voyages()
